...
FIXME : pour behavior set_fifo_error: j'ai du modifier la spec de usbotghs_set_xmit_fifo pour faire assigns usbotghs_ctx (si j'étais trop précis dans les
        assigns, ça ne passait pas ici...) donc on peut être plus précis, normalement assigns \nothing doit être prouvé pour ce behavior

        la fonction usbotghs_write_epx_fifo(ep->mpsize, ep) : si je ne la commente pas, assigns de fifo_no_error ne passe pas
...
mbed_error_t usbotghs_send_data(uint8_t *src, uint32_t size, uint8_t ep_id)
{
    ...
    residual_size = 1024 ; // cyril : ajout pour rentrer dans la boucle
...
mbed_error_t usbotghs_deconfigure_endpoint(uint8_t ep)
{
    ...
    if (ctx->in_eps[ep].configured == true) {  // Cyril : RTE : aucun test sur ep, donc ça peut dépasser la taille du tableau
        clear_reg_bits(r_CORTEX_M_USBOTG_HS_DIEPCTL(ep),
                USBOTG_HS_DIEPCTL_EPENA_Msk);
    }
    if (ctx->out_eps[ep].configured == true) {  // Cyril : RTE : aucun test sur ep, donc ça peut dépasser la taille du tableau
...
/* sanitation */
    ...
    //usbotghs_write_core_fifo(&(ep->fifo[ep->fifo_idx]), size, ep->id);  // Cyril : commenté car pb de cast et d'accès mémoire
      ...
      /* 
        RTE : je ne le voyais pas avant car la fonction usbotghs_write_core_fifo ne termine pas correctement 
      assert rte: unsigned_overflow: ep->fifo_idx + size ≤ 4294967295; 
    */
